...
namespace GuiInGame
{
    public class GUI : MonoBehaviour
    {
        ...
        public void ChangeAnimationState(string trigger)
        {
            ...
            AnalyticsManager.instance.LogEvent("Changed_MainMenu_Trigger", new Dictionary<string, string>
            {
                {
                    "Trigger", trigger
                }
        ...
        public void UpdateExperience()
        {
            ...
            {
                levelProgress.fillAmount = (float)((DataLoader.playerData.experience - DataLoader.Instance.levelExperience[currentPlayerLevel - 1]) /
                    (DataLoader.Instance.levelExperience[currentPlayerLevel] - DataLoader.Instance.levelExperience[currentPlayerLevel - 1]));
            }
        }
        ...
        public void TryToShowInterstirial()
        {
            if (DataLoader.playerData.gamesPlayed > 3 && heroesInQueue.Count == 0 && !isNewWorldOpened)
            {
                ...
                if(interstitialAdsCounter >= 1)
                {
                    ...
                    BridgeController.instance.ShowInterstitial($"Show Interstitial{AdName.Interstitial}", onDone);
                }
            }
        }
        ...
        public void CheckOpenedHeroes()
        {
            ...
            if (newOpenedHeroes.Count > 0)
            {
                ...
                UIController.instance.scrollControllers.survivorsController.scrollRect.content.anchoredPosition =
                    new Vector2(0f, UIController.instance.scrollControllers.survivorsController.scrollRect.content.anchoredPosition.y);
        ...
        {
            if (GameManager.instance.currentGameMode != GameManager.GameModes.PVP && pause && pauseReady && !GameManager.instance.isTutorialNow &&
                PlayerPrefs.HasKey(StaticConstants.AbilityTutorialCompleted))
            {
        ...
        {
            if (GameManager.instance.IsWorldOpen(GameManager.instance.currentWorldNumber + 1) && (!PlayerPrefs.HasKey(StaticConstants.GoToNextWorldPopUpShowed) ||
                PlayerPrefs.GetInt(StaticConstants.GoToNextWorldPopUpShowed) <= GameManager.instance.currentWorldNumber))
            {
        ...
        public void GetOfflineMoney()
        {
            ...
            if (PlayerPrefs.HasKey(StaticConstants.UpgradeTutorialCompleted) && num / 60.0 > (double)StaticConstants.MinOfflineMinutes && heroesInQueue.Count == 0)
            {
                ...
                double f = Math.Floor((float)(num * (double)num2 * (double)StaticConstants.OfflineGoldConst * (double)(1f + PassiveAbilitiesManager.bonusHelper.GoldBonus)) *
                                      (1f + PassiveAbilitiesManager.bonusHelper.DamageBonus) * (1f + PassiveAbilitiesManager.bonusHelper.AttackSpeedBonus) *
                                      (1f + PassiveAbilitiesManager.bonusHelper.GetCriticalHitChance()));
                ...
                offlineX2.onClick.AddListener(delegate
                {
                    ...
                    BridgeController.instance.ShowRewarded($"Show reward: {AdName.RewardX3Offline}", onDone);
            ...
            {
                Debug.Log(
                    "Sorry, offline time < " + StaticConstants.MinOfflineMinutes + " min, money won't be scored. You can change minimal time in StaticConstants.cs -> MinOfflineMinutes");
        ...
        public void RefreshArenaTimeRemaining(int seconds, bool setFull = false)
        {
            ...
            fillFuelTank.rectTransform.sizeDelta =
                new Vector2((1f - (float)seconds / (float)ArenaWavesManager.instance.timeBeforeStart) * maxWidthFuelTank, fillFuelTank.rectTransform.sizeDelta.y);
        }
        ...
        public void RefreshProgressToBoss()
        {
            ...
            {
                fillProgressToBoss.rectTransform.sizeDelta =
                    new Vector2(
                        (GameManager.instance.Points - (float)WavesManager.instance.GetBossDeadAtPoints()) / (float)WavesManager.instance.GetNextBossTargetPoints() * maxWidthProgressToBoss,
                        fillProgressToBoss.rectTransform.sizeDelta.y);
            }
            else
            {
        ...
        public void RefreshWorldsButtons()
        {
            ...
            {
                textButtonNewLocation.text = GameManager.instance.bossKillsForOpenWorld[1] - DataLoader.playerData.GetZombieByType(SaveData.ZombieData.ZombieType.BOSS).totalTimesKilled +
                    string.Empty;
